function R=Radon_inverse(t,delta,M,p,weights,ref_dist,line_model,inversion_model,hyperparameters)
  ...
  for i=1:floor((iF+1)/2)
      ...
      if(strcmpi(inversion_model, 'Cauchy')||strcmpi(inversion_model, 'L1'))
          ...
          elseif(strcmpi(inversion_model, 'L1'))
              COST_prev=norm(Mfft(:,i)-A*Rfft(:,i), 2)+lambda*norm(abs(Rfft(:,i))+b, 1);
          end;
          iter=1;
          ...
          while( (dCOST > 0.001)&&(iter<20) )
              ...
              elseif(strcmpi(inversion_model, 'L1'))
                  COST_cur=norm(Mfft(:,i)-A*Rfft(:,i), 2)+lambda*norm(abs(Rfft(:,i))+b, 1);
              end;
